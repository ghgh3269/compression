...
def synthesis_transform(tensor, num_filters):
  ...
  with tf.variable_scope("synthesis"):
    with tf.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
          1, (5, 5), corr=False, strides_up=2, padding="same_zeros",
          channel_separable=True, use_bias=True, activation=tfc.GDN(inverse=True))
      conv1_1 = tfc.SignalConv2D(
          num_filters, (1, 1), corr=False, strides_up=1, padding="same_zeros",
          use_bias=False, activation=None)
      tensor = layer(tensor)
      tensor = conv1_1(tensor)
    ...
    with tf.variable_scope("layer_2"):
      layer = tfc.SignalConv2D(
          ...
          channel_separable=False, use_bias=True, activation=None)
      tensor = layer(tensor)
