...
def get_batch(dataset, num_train):
  ...
  for i in range(args.batchsize):
    ...
    # wavelet transform
    tmp_wave = rgb2wave(tmp)

    batch.append(tmp) 
    batch_wave.append(tmp_wave)
  return batch, batch_wave

def rgb2wave(x):
  ''' HxWX3 => H/2 x W/2 X 12'''
  wave_list = []
  for i in range(3):
      cA, (cH, cV, cD) = pywt.dwt2(x[:,:,i], 'haar')
      cA = np.expand_dims(cA, axis=-1)
      cH = np.expand_dims(cH, axis=-1)
      cV = np.expand_dims(cV, axis=-1)
      cD = np.expand_dims(cD, axis=-1)
      
      wave = np.concatenate((cA, cH, cV, cD), axis=-1)
      wave_list.append(wave)
  return np.concatenate(wave_list, axis=-1)
...
def conv(x, num_feats, kernel_size=[3,3], strides=(1,1), padding='same', activation=None, kernel_initializer=None):
  kernel_initializer = tf.contrib.layers.variance_scaling_initializer(factor=0.0001/10, mode="FAN_IN", uniform=False)
...
def MAM(x, num_feats, ratio=16):
  with tf.variable_scope('VCM'):
      _, tmp_var = tf.nn.moments(x, axes=[1,2], keep_dims=True)
      tmp_var = tf.sqrt(tmp_var)
      tmp_ca = conv(tmp_var, num_feats//ratio, [1,1], activation=tf.nn.relu)
      tmp_ca = tmp_var + conv(tmp_ca, num_feats, [1,1])
...
def analysis_transform(tensor, wave, num_filters):
  """Builds the analysis transform."""

  with tf.variable_scope("analysis"):

    with tf.variable_scope("init_layer"):
      conv1 = tfc.SignalConv2D(
    ...
    with tf.variable_scope("layer_0"):
      ...
      conv2 = tfc.SignalConv2D(
          num_filters, (3, 3), corr=True, strides_down=1, padding="same_zeros",
          use_bias=True, activation=None)
      layer = tfc.SignalConv2D(
          num_filters, (9, 9), corr=True, strides_down=2, padding="same_zeros",
          use_bias=True, activation=None)
      tensor = tensor + conv2(conv1(tensor))
      tensor = layer(tensor)
      tensor_ = tf.concat([tensor, wave], axis=-1)
    ...
    with tf.variable_scope("layer_1"):
      ...
      layer = tfc.SignalConv2D(
          ...
          use_bias=True, activation=None)
      tensor = tensor + conv2(conv1(tensor_))
      tensor = layer(tensor)
...
def train():
  ...
  crop_shape = (args.batchsize, args.patchsize, args.patchsize, 3)
  crop_shape_ = (args.batchsize, args.patchsize/2, args.patchsize/2, 3*4)

  x = tf.placeholder(tf.float32, crop_shape)
  wave = tf.placeholder(tf.float32, crop_shape_)
  num_pixels = np.prod(crop_shape[:-1])

  # Build autoencoder.
  y = analysis_transform(x, wave, args.num_filters)
  ...
  with tf.Session() as sess:
    ...
    for _ in range(args.last_step):
      img_batch, wave_batch = get_batch(dataset, len(img_names))
      _, train_summary, loss, global_step = sess.run([train_op, merged, train_loss, step], feed_dict={x: img_batch, wave: wave_batch})
...
def compress():
  ...
  wave = tf.placeholder(tf.float32, [1, None, None, 12])
  ...
  with tf.Session() as sess:
    ...
    for img, img_name in zip(dataset, img_names):
      wave_batch = rgb2wave(img)
      # Get the compressed string and the tensor shapes.
      _string, x_shape, y_shape = sess.run([string, tf.shape(x), tf.shape(y)], feed_dict={x: [img], wave: [wave_batch]})
